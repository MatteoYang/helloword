...
<body>
							...
							<dd>
								<c:forEach var="towFn" items="${oneFn.childs }">
									...
									<dl class="cl permission-list2">
										...
										<dd>
											<c:forEach var="btn" items="${towFn.sysFnBtnRel}" varStatus="each" >
												<c:set value="${count+1 }" var="count"></c:set>
												<!-- 二级功能按钮 -->
												<div class="checkbox checkbox-inline checkbox-info">
													<input type="checkbox" value="${btn.btnValue}" name="fnList[${index}].sysFnBtnRel[${each.index}].btnValue"
													<c:if test="${btn.hasThisBtn}">  checked="checked"  </c:if>
														id="checkbox${count}"> 
														<label for="checkbox${count}"> <c:out value="${btn.btnKey}"></c:out>
													</label>
												</div>
											</c:forEach>
										</dd>
	...
	var invokeUrl="${path}/do/admin/sysRole/su/addRole";
	<c:if test="${obj ne null }">
	invokeUrl = "${path}/do/admin/sysRole/su/modifyRole";
	</c:if>
