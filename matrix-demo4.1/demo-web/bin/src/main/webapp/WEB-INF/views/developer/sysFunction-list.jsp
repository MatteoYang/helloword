...
<body class="gray-bg">
		...
		$('#fnIsDisable').change(function () {
			if(flag){
				...
				if (id != null && id != '') {
					...
					}).invoke("${path}/do/admin/sysFunction/"+path,
							function(loj) {
	...
	function initFunctionSelected(id) {
		...
		$.AjaxProxy().invoke("${path}/do/admin/sysFunction/all",
			function(loj) {
	...
	function updateFunction() {
		var myForm = MForm.initForm({
			invokeUrl : "${path}/do/admin/sysFunction/modifyFunction",
	...
	function zTreeOnClick(event, treeId, treeNode) {
		...
		$.AjaxProxy({
				...
				.invoke(
						"${path}/do/admin/sysFunction/findById",
						function(loj) {
							// 初始化功能基本信息1
											...
											function(loj2) {
												...
												for (var i = 0; i < loj2
													...
													if (isbtnselect) {
														...
														html[i]= '<div class="checkbox ">  <input  name="sysFnBtnRel['+i+'].btnValue" type="checkbox" checked="checked" id="inlineCheckbox' 
																...
																+'<input value="'+ fbPath+ '" type="text" name="sysFnBtnRel['+i+'].fbPath"  class="form-control inline"    maxlength="200">'+'</div>'
																+'<input type="hidden"  name="sysFnBtnRel['+i+'].btnKey" value="'+ loj2.getString(i,"btnKey")+'"  />';
													} else {
														html[i]= '<div class="checkbox ">  <input  name="sysFnBtnRel['+i+'].btnValue" type="checkbox" id="inlineCheckbox'
																...
																+ ' &nbsp;&nbsp;</label> '
																+'<input value="'+ loj2.getString(i,"fbPath")+ '" type="text" name="sysFnBtnRel['+i+'].fbPath" class="form-control inline"    maxlength="200"></div>'
																+'<input type="hidden"  name="sysFnBtnRel['+i+'].btnKey" value="'+ loj2.getString(i,"btnKey")+'"  />';
													}
												}
												$("#btns").html(html.join(""));
	...
	function initFunctionList() {
		...
		function showIcon() {
			...
			area : [ '80%', '80%' ],
				...
				shadeClose: true,
				  shade: 0.8,
				maxmin : true,
				content : '${path}/do/common/redirect/icons'
			});
		}
		...
		function delFunction() {
			...
			if (id != null && id != '') {
				layer.confirm('删除功能时会删除该功能的所有子功能！', {
					...
					}).invoke("${path}/do/admin/sysFunction/del", function(loj) {
						layer.msg('删除成功！', {
							icon : 1,time:1000
						}, function(index) {
							$("#dataform").hide();
		...
		function openForm() {
			layer.open({
				...
				content : [ '${path}/do/admin/sysFunction/editForm' ]
			});
		}
	</script>
