			...
			<div class="form-group">
				...
				<div class="col-sm-9" id="btns">
					<c:forEach items="${btnList }" var="item" varStatus="count">
							<div class="checkbox ">
							 <input  name="sysFnBtnRel[${count.index}].btnKey" type="hidden" value="${item.btnKey }"> 
								<input name="sysFnBtnRel[${count.index}].btnValue" type="checkbox"
									id="inlineCheckbox${item.btnValue }"
									value="${item.btnValue }"> <label
									for="inlineCheckbox${item.btnValue }"> ${item.btnKey }
								</label>
								<input  type="text" name="sysFnBtnRel[${count.index}].fbPath" class="form-control inline"    maxlength="200">
							</div>
					</c:forEach>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			myForm = MForm.initForm({
				invokeUrl : "${path}/do/admin/sysFunction/addFunction",
